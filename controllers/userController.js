...
exports.login = async (req, res) => {
    ...
    let email = request.email;
    let password = request.password;
    const checkEmail = await User.findOne({ email: req.body.email, password: password, isVerified: true });
    if (checkEmail === null) {
...
exports.showAllBookings = async (req, res) => {
    ...
    let bookings = await Booking.find({ lawyerId: req.body.id }).sort({ createdAt: -1 }).populate("userId").populate("lawyerId");
    return res.status(200).json(bookings);
}

exports.showUserBookings = async (req, res) => {

    let bookings = await Booking.find({ userId: req.body.id }).sort({ createdAt: -1 }).populate("userId").populate("lawyerId");
    return res.status(200).json(bookings);
}
